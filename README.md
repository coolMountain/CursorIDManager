...
3. 打开终端，执行以下命令：
   ...
   ```bash
   sudo bash <(curl -fsSL https://raw.githubusercontent.com/hamflx/cursor-reset/main/reset.sh)
   ```
...
```bash
sudo bash <(curl -fsSL https://raw.githubusercontent.com/hamflx/cursor-reset/main/reset.sh) --restore
```
